...
// g++ -std=c++17 gauss_seidel_sequential.cpp -o gauss_seidel_sequential
...
float GaussSeidelSequential::ComputeSquaredResidual(const std::vector<double> &u, const std::vector<double> &source){
    double value = 0.0;
    double dx2 = dx_ * dx_;
    double dy2 = dy_ * dy_;
    for (int i = 1; i <= Nx_; i++)
...
float GaussSeidelSequential::ComputeSquaredError(const std::vector<double> &u, const std::vector<double> &u_exact){
    double value = 0.0;
    // double tmp = 0.0;
    for (int i = 0; i <= Nx_ + 1; i++)
    {
...
void GaussSeidelSequential::ComputeOneStep(const std::vector<double>& u, std::vector<double>& new_u, const std::vector<double>& source) {
    ...
    for (int i = 1; i <= Nx_; i++) {
        for (int j = 1; j <= Ny_; j++) {
            new_u[I(i, j)] = (1. / A_ii) * (- (u[I(i + 1, j)] + new_u[I(i - 1, j)]) / dx2 - (u[I(i, j + 1)] + new_u[I(i, j - 1)]) / dy2 + source[I(i, j)]);
        };
    };
};
...
void GaussSeidelSequential::SaveOnDomain(const std::vector<double> &field, std::string file_name, std::string format) {
...
double left_condition(double y){
...
double null_function(double x, double y){
    return 0.0;
}
...
int main() {
    ...
    GaussSeidelSequential gauss_seidel_seq(
        n, n, x_min, x_max, y_min, y_max, 
        left_condition, dirichlet_condition, 
        dirichlet_condition, dirichlet_condition, 
        constant, constant, nullptr, false);
...
//     // --------------------------------
//     std::filesystem::path full_name = "./results/gauss_seidel_sequential_asymptotic_accuracy.csv";
