...
#include "gauss_seidel_mpi.hpp"

GaussSeidelMPI::GaussSeidelMPI(
...
void GaussSeidelMPI::CheckMPIError(int errCode, const std::string& errMsg) {
...
void GaussSeidelMPI::InitializeLocalProblem(
    OneVarFuncPtr left_condition_func, OneVarFuncPtr right_condition_func,
    OneVarFuncPtr top_condition_func, OneVarFuncPtr bottom_condition_func,
    TwoVarsFuncPtr source_func, TwoVarsFuncPtr init_guess_func,
...
int GaussSeidelMPI::I(int i, int j)
{
    return i + j * (Nx_ + 2);
}

bool GaussSeidelMPI::IsRed(int i, int j){
    return (i + j) % 2 == 0;
}

double GaussSeidelMPI::Ax(const std::vector<double> &u, int i, int j){
    return (u[I(i + 1, j)] - 2. * u[I(i, j)] + u[I(i - 1, j)]) / dx2_ + (u[I(i, j + 1)] - 2. * u[I(i, j)] + u[I(i, j - 1)]) / dy2_;
}

double GaussSeidelMPI::Nx(const std::vector<double> &u, int i, int j) {
    return -(u[I(i + 1, j)] + u[I(i - 1, j)]) / dx2_ - (u[I(i, j + 1)] + u[I(i, j - 1)]) / dy2_;
}

double GaussSeidelMPI::LocalSquaredResidual(const std::vector<double> &u, const std::vector<double> &source){
    double value = 0.0;
    for (int i = 1; i <= Nx_; i++)
    {
...
double GaussSeidelMPI::LocalInitialSquaredResidual(const std::vector<double> &source){
    double value = 0.0;
    for (int i = 1; i <= Nx_; i++)
    {
        for (int j = 1; j <= num_local_rows_; j++){
...
double GaussSeidelMPI::LocalSquaredError(const std::vector<double> &u, const std::vector<double> &u_exact){
    double value = 0.0;
...
void GaussSeidelMPI::ComputeOneLocalStep(const std::vector<double> &u, std::vector<double> &new_u, const std::vector<double> &source){
    ...
    {
        for (int j = 1; j <= num_local_rows_; j++) {
            if (IsRed(i,j + j_start_ - 1)){
                new_u[I(i, j)] = A_ii_minus_one * (source[I(i, j)] + Nx(u, i, j));
            }
            
        };
    };
    ExchangeNewBoundaries();
    // compute the black points
    for (int i = 1; i <= Nx_; i++)
    {
        for (int j = 1; j <= num_local_rows_; j++) {
            if (!IsRed(i,j + j_start_ - 1)){
                new_u[I(i, j)] = A_ii_minus_one * (source[I(i, j)] + Nx(new_u, i, j));
            }
        };
    };
}
...
void GaussSeidelMPI::Solve(int max_iterations, double epsilon, bool verbose) {
    bool converged = false;
    MPI_Barrier(MPI_COMM_WORLD);  // Initial sync
    ...
    for (int l = 1; l <= max_iterations; l++) {
        ...
        CheckMPIError(MPI_Bcast(&converged, 1, MPI_C_BOOL, 0, MPI_COMM_WORLD), "Error broadcasting convergence signal");
    }
    ...
    if (!local_exact_sol_.empty()) {
        double local_squared_error = LocalSquaredError(local_sol_, local_exact_sol_);
        CheckMPIError(MPI_Reduce(&local_squared_error, &error_, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD),
                      "Error reducing global squared error");
        if (process_rank_ == 0) {
...
void GaussSeidelMPI::GatherGlobalSolution() {
...
void GaussSeidelMPI::ExchangeBoundaries() {
    if (process_rank_ > 0) {
        MPI_Sendrecv(
            &local_sol_[I(0, 1)],
            Nx_ + 2,
...
void GaussSeidelMPI::ExchangeNewBoundaries() {
    if (process_rank_ > 0) {
        MPI_Sendrecv(
            &new_local_sol_[I(0, 1)],
            Nx_ + 2,
            MPI_DOUBLE,
            process_rank_ - 1,
            0,
            &new_local_sol_[I(0, 0)],
            Nx_ + 2,
            MPI_DOUBLE,
            process_rank_ - 1,
            0,
    ...
    if (process_rank_ < num_processes_ - 1) {
        MPI_Sendrecv(
            &new_local_sol_[I(0, num_local_rows_)],
            Nx_ + 2,
            MPI_DOUBLE,
            process_rank_ + 1,
            0,
            &new_local_sol_[I(0, num_local_rows_ + 1)],
            Nx_ + 2,
            MPI_DOUBLE,
            process_rank_ + 1,
            0,
...
void GaussSeidelMPI::SaveOnDomain(const std::vector<double> &field, std::string file_name, std::string folder, std::string format){
